...
- Creación y edición manual de máquinas (cada COTI = una fila).
...
| Término | Significado |
| --- | --- |
| **COTI** | Identificador único por máquina (no por orden). Una misma compra de un cliente puede tener varias COTIs, una por cada máquina. |
...
-- Cada fila = una máquina (= una COTI) en producción
create table machines (
  id uuid primary key default gen_random_uuid(),
  coti_number integer not null,                   -- 977, 988, 999, etc.
...
- Cada fila muestra:
  - **COTI** y **código del equipo** (p.ej. `#977 · XM165`).
...
- Búsqueda rápida por COTI o por nombre en la parte superior (opcional).
...
**Elementos:**
- Encabezado con: COTI, código, nombre del equipo, cliente, ciudad, color, fecha prometida y avance total.
...
5. **Máquinas en producción** desde la hoja `plan de producción`, una fila por cada COTI, preservando el orden actual como `order_position`. El estado actual de avance de cada etapa se infiere de las celdas marcadas con `x` o con texto:
