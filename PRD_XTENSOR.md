...
- Creación y edición manual de máquinas (cada PLACA = una fila).
...
| Término | Significado |
| --- | --- |
| **PLACA** | Identificador único por máquina (no por orden). Una misma compra de un cliente puede tener varias PLACAs, una por cada máquina. |
...
-- Cada fila = una máquina (= una PLACA) en producción
create table machines (
  id uuid primary key default gen_random_uuid(),
  placa_number integer not null,                   -- 977, 988, 999, etc.
...
- Cada fila muestra:
  - **PLACA** y **código del equipo** (p.ej. `#977 · XM165`).
...
- Búsqueda rápida por PLACA o por nombre en la parte superior (opcional).
...
**Elementos:**
- Encabezado con: PLACA, código, nombre del equipo, cliente, ciudad, color, fecha prometida y avance total.
...
- **PLACA** (numérico, requerido, único)
...
5. **Máquinas en producción** desde la hoja `plan de producción`, una fila por cada PLACA, preservando el orden actual como `order_position`. El estado actual de avance de cada etapa se infiere de las celdas marcadas con `x` o con texto:
